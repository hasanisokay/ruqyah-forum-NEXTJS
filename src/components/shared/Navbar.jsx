...
const Navbar = () => {
  ...
  const { fetchedUser, loading, logOut, loggedOut } = useContext(AuthContext);
  ...
  if (!loading) return (
    <div className="flex md:px-10 px-4 justify-between items-center" ref={navRef}>
      <Link href={"/"}><Image placeholder="blur" src={logo} alt="logo" width={150} /></Link>

      <div
        className={`absolute ${navToggle ? "right-0" : "left-[-120%]"
          } top-[4.5rem] w-[40vw] flex justify-center items-center bg-slate-200 py-3 rounded-xl transition-all duration-500 dark:bg-slate-900 lg:static lg:w-[unset] lg:flex-row lg:bg-transparent lg:pb-0 lg:pt-0 dark:lg:bg-transparent`}
      >
